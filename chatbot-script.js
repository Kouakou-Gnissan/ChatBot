...
const closeChabot = document.getElementById("close-chatbot");
...
const API_KEY = "AIzaSyBjLkPjRkI8LK7RSsPW--EhsOnxFWJthg8";
...
//generer la reponse du bot avec l'API
const generateBotReponse = async (incomingMessageDiv) => {
    ...
    try {
        const reponse = await fetch(API_URL, requestOptions);
        const data = await reponse.json();
        if (!reponse.ok) throw new Error(data.error.message);
        ...
        // donner une memoire au bot
        chatHistory.push({
            role: "model",
            parts: [{ text: userData.message }]
        });

    } catch (error) { // en cas d'erreur (Non validité de l'API ou autres)
...
// ajusterle scrollbar de la zone de message 
...
sendMessageButton.addEventListener("click", (e) => handOutgoingMessage(e));
